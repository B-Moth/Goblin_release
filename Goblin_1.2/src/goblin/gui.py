...
from goblin.ollama_runtime import (
    check_ollama_model,
...
def create_app(output_dir: str = "transcriptions", offline: bool = False) -> Flask:
    ...
    def ollama_pull_status():
        ...
        if not model:
            return jsonify({'ok': False, 'error': 'model is required'}), 400
        return jsonify(get_ollama_pull_status(model, app.ollama_jobs))
...
def run_gui(output_dir: str = "transcriptions", host: str = "127.0.0.1", port: int = 5000, offline: bool = False) -> None:
    ...
    # Auto-pull Qwen model on first use (offline mode expects this)
    if offline:
        from goblin.ollama_runtime import ensure_ollama_model_available
        try:
            print("Vérification du modèle Qwen local...")
            ensure_ollama_model_available("qwen2.5:7b-instruct")
        except RuntimeError as e:
            print(f"⚠ {e}")
            print("ℹ Vous pouvez toujours utiliser le mode en ligne avec une clé API OpenAI.")
    ...
    def _watch_browser_activity() -> None:
        while not watchdog_stop.wait(1.0):
            ...
            if time.monotonic() - last_seen > browser_shutdown_timeout:
                # TEMPORARILY DISABLED: Heartbeat check
