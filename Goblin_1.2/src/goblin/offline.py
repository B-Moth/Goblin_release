...
def preload_whisper_model(model_size: str = "base") -> None:
    """Pre-download and cache a whisper model for offline use.
    
    Args:
        model_size: Model size to preload ("tiny", "base", or "medium").
    """
    global _WHISPER_MODEL_SIZE, _whisper_model
    
    if model_size not in {"tiny", "base", "medium"}:
    ...
    logger.info("Pre-downloading Whisper « %s » model…", model_size)
    # Load the model to trigger download and caching
    WhisperModel(model_size, device="cpu", compute_type="int8")
    logger.info("✓ Whisper « %s » model ready.", model_size)
